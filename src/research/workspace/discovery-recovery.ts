...
interface DiscoveryRecoveryContractBase {
  projectId: string;
...
export interface LegacyDiscoveryRecoveryContract extends DiscoveryRecoveryContractBase {
  schemaVersion: 1;
}

export interface DiscoveryRecoveryContractV2 extends DiscoveryRecoveryContractBase {
  schemaVersion: 2;
  floorClosureAction: "reject-further-citation-chase-formalization-and-admission";
}

export type DiscoveryRecoveryContract =
  | LegacyDiscoveryRecoveryContract
  | DiscoveryRecoveryContractV2;
...
export function parseDiscoveryRecovery(value: unknown): DiscoveryRecoveryContract {
  ...
  const keys =
    isObject(value) && value.schemaVersion === 1
      ? commonKeys
      : isObject(value) && value.schemaVersion === 2
        ? [...commonKeys, "floorClosureAction"]
        : [];
  if (
    !isObject(value) ||
...
export async function assertDiscoveryRecoveryObjectBinding(
  ...
  const expected = {
    ...
    ...(contract.schemaVersion === 2 ? { floorClosureAction: contract.floorClosureAction } : {}),
  ...
  const actual = {
    ...
    ...(binding.schemaVersion === 2 ? { floorClosureAction: binding.floorClosureAction } : {}),
